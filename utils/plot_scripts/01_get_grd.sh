...
outmodel=../results/mod_iter5.dat
do_syn_test=true
...
do
    ...
    if [ $do_syn_test ];then 
        awk -v a=$dep '($3 - a <= 1.0e-4 && $3-a >=-1.0e-4) {print $1,$2,$4}' ../results/mod_true.dat >slice/depth.${dep}.true.dat
        awk -v a=$dep '($3 - a <= 1.0e-4 && $3-a >=-1.0e-4) {print $1,$2,$4}' ../results/mod_iter0.dat >slice/depth.${dep}.init.dat
    fi
done 
...
do
    ...
    if [ $do_syn_test ] ;then 
        gmt surface slice/depth.$dep.true.dat -Ggrd/depth.$dep.true.grd  -I$dx/$dy -R$region
        gmt surface slice/depth.$dep.init.dat -Ggrd/depth.$dep.init.grd  -I$dx/$dy -R$region
