...
subroutine aprod(mode, m, n, x, y, lenrw, rw,col,val)
   ...
   use,intrinsic :: iso_c_binding
   implicit none
   integer(c_int),intent(in) :: mode,m,n,lenrw
   integer(c_int),INTENT(IN) :: rw(lenrw),col(lenrw)
   real(dp),INTENT(IN) :: val(lenrw)
   ...
   if(mode == 1) then
      do k=1,lenrw
         y(rw(k)) = y(rw(k)) + val(k) * x(col(k))
      enddo
   else
      do k=1,lenrw
         x(col(k)) = x(col(k)) + val(k) * y(rw(k))
      enddo
   endif
end
end
...
subroutine LSMR(m,n,lenrw,rw,col,val,b, damp,atol, btol, conlim, itnlim, &
               localSize,x, istop, itn, normA,&
               condA, normr, normAr, normx ) bind(C,name="LSMR")
!subroutine LSMR  ( m, n, leniw, lenrw,iw,rw, b, damp,               &
!                     atol, btol, conlim, itnlim, localSize, nout,      &
!                     x, istop, itn, normA, condA, normr, normAr, normx )
   use lsmr_coo_matrix
   integer(c_int),value, intent(in) :: lenrw,m,n
   integer(c_int), INTENT(IN) :: rw(lenrw),col(lenrw)
   real(dp),INTENT(IN) :: val(lenrw)
